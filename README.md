...
This software was created at Helmholtz-Zentrum Geesthacht, Zentrum für Material- und Küstenforschung (HZG) and turned into a collaboration with Bundesanstalt für Materialforschung und -prüfung (BAM).
...
By default, the following axis designations are used in all output and evaluation files:

+ `"x"` for the detector's row axis (horizontal stage axis),
+ `"y"` for the detector's column axis (vertical stage axis / rotation axis),
+ `"z"` for the principal beam axis.

To change these designations to match your setup, call the following functions on your `ballSequence` object:
...
Blue dots represent the results for the ball centers determined by the circle fit (or intensity fit, if activated). In the two upper subgraphs, the orange lines show the fit function (either 1st or 2nd order sine waves). The dashed red line in the upper subgraph displays the location of the calculated rotation axis. In the bottom graph, the orange line displays the trajectory calculated only from the 1st order part of the fit function (i.e. the sine wave with a frequency of the stage rotation frequency). If second order wave fitting is activated, the dark green line displays the trajectory calculated from the complete fit function (1st and 2nd order components). For intensity fit plots, orange lines are replaced by gray lines for better distinction. Their meaning remains the same.
